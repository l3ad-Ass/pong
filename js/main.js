...
function Player(number, ground) {
	...
	this.color;
	if (number == 1) {
		this.x = ground.margin + this.width / 2 + 2;
		this.y = ground.height / 2 + ground.margin + this.height / 2 - this.height / 2;
		this.color = '#f0f0f0';
	} else if (number == 2) {
		this.x = ground.margin + ground.width - (2 + this.width / 2);
	...
	this.update = function(newY) {
		...
		if(newY>ground.y+this.height/2&&newY<ground.y+ground.height-this.height/2){	
			this.y=newY;
		}
	};
}
